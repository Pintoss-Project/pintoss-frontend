...
const AddProductBox = ({ productId, setSelectedProductId, setIsEditing, isEditing }: Props) => {
  ...
  const { data: productDetails, isSuccess } = useQuery({
    ...
    queryFn: () => (productId ? getProduct(productId) : Promise.reject('No product ID provided')),
    enabled: !!productId,
  });
  ...
  const { data: priceCategoryList } = useQuery({
    ...
    queryFn: () =>
      productId ? getPriceCategoryList(productId) : Promise.reject('No product ID provided'),
    enabled: !!productId,
  });
  ...
  const deletePriceCategoryMutation = useMutation({
    mutationFn: (categoryId: number) =>
      productId ? deletePriceCategory(productId, categoryId) : Promise.reject('No product ID'),
    onSuccess: () => {
      open({
        width: '300px',
        height: '200px',
  ...
  const updateProductMutation = useMutation({
    mutationFn: (data: ProductInfoFormData) =>
      productId ? updateProduct(productId, data) : Promise.reject('No product ID'),
    onSuccess: () => {
      open({
        width: '300px',
        height: '200px',
